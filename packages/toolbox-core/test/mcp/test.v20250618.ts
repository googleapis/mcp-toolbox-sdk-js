...
describe.each([
...
])('McpHttpTransportV20250618 [%s]', telemetryEnabled => {
  const testBaseUrl = 'http://test.loc';
  let mockSession: jest.Mocked<AxiosInstance>;
  let transport: McpHttpTransportV20250618;
  ...
  beforeEach(() => {
    ...
    transport = new McpHttpTransportV20250618(
      testBaseUrl,
      mockSession,
  ...
  describe('initialization', () => {
    ...
    it('should use provided client name and version in handshake', async () => {
      const customTransport = new McpHttpTransportV20250618(
        testBaseUrl,
        mockSession,
        Protocol.MCP_v20250618,
        'custom-client',
  ...
  describe('toolInvoke', () => {
    ...
    it('should not warn if using HTTPS', async () => {
      ...
      const httpsTransport = new McpHttpTransportV20250618(
